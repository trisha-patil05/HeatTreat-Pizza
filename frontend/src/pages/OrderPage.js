...
function OrderSummary() {
  ...
  // ✅ Snapshot — cart clear hone ke baad bhi bill dikhega
  const [snapshot] = useState({
    items:      cartItems,
    subtotal:   getCartTotal(),
    gst:        Math.round(getCartTotal() * 0.18),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      localStorage.setItem('orderData', JSON.stringify({
        ...
        address,
        payment,
        size:       snapshot.items[0]?.size     || 'Medium',
        toppings:   snapshot.items[0]?.toppings || [],
        drink:      snapshot.items[0]?.drink    || 'Coke',
        totalPrice: snapshot.grandTotal,
      }));

      setOrderId(savedOrderId);
      clearCart(); // ✅ Cart clear AFTER saving everything
      setConfirmed(true);

    } catch (err) {
      console.error("❌ Order Error:", err.response || err);
      setApiError(err.response?.data?.message || "Order place nahi hua. Dobara try karo.");
    } finally {
      setLoading(false);
    }
  };

  // ── Empty Cart (sirf tab jab confirmed nahi hua) ──
  if (!confirmed && snapshot.items.length === 0) {
    return (
      ...
      <div className="order-page">
        <div className="order-container" style={{ textAlign: 'center' }}>
          ...
          <div className="order-confirm-box">
            <p><span>Order ID</span><strong style={{ fontFamily: 'monospace', fontSize: '13px' }}>#{String(orderId).slice(-8).toUpperCase()}</strong></p>
            ...
            <p><span>Grand Total</span><strong style={{ color: 'var(--orange)' }}>₹{snapshot.grandTotal}</strong></p>
          </div>

          <div style={{ display: 'flex', gap: '12px', marginTop: '24px', flexWrap: 'wrap' }}>
            {/* ✅ Summary pe navigate */}
            <button className="btn-primary" style={{ flex: 1 }} onClick={() => navigate('/summary')}>
              📄 View Receipt
            </button>
            <button className="btn-outline" style={{ flex: 1 }} onClick={() => navigate('/my-orders')}>
              🧾 My Orders
            </button>
            <button className="btn-outline" style={{ flex: 1 }} onClick={() => navigate('/home')}>
              🍕 Order More
            </button>
          </div>
        </div>
        ...
        <div className="order-two-col">
          ...
          <div className="order-left">
            ...
            <div className="bill-breakdown" style={{ marginTop: '16px' }}>
              <div className="bill-row"><span>Subtotal</span><span>₹{snapshot.subtotal}</span></div>
              <div className="bill-row"><span>GST (18%)</span><span>₹{snapshot.gst}</span></div>
              <div className="bill-row total-row"><span>Grand Total</span><span>₹{snapshot.grandTotal}</span></div>
            </div>
          </div>
          ...
          <div className="order-right">
            <div className="input-group">
              ...
              />
              {addressError && (
                <p style={{ color: '#ff4444', fontSize: '13px', marginTop: '6px' }}>{addressError}</p>
              )}
            </div>
            ...
            <div className="input-group">
              ...
              <div className="payment-options">
                {paymentMethods.map((method) => (
                  <label
                    key={method.id}
                    className={`payment-option ${payment === method.id ? 'selected-payment' : ''}`}
                  >
                    <input
                      type="radio"
                      name="payment"
